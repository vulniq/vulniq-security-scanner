...
public class UnixFileTestImplTest extends TerziTestUtilizingDocker
{
    ...
    @Test
    public void testUnixFileTestDocker()
    {
        ...
        String ovalXMLPath = absolutePath + "/src/main/resources/unit-test-xml/unix-def_file_test.xml";
        File ovalFile = new File(ovalXMLPath);
        
        OvalDefinitions definitions = OVALXMLLoader.loadOVALDefinitionsFile(ovalFile);
        //ovalInterpreter.evaluateTest("oval:org.mitre.oval.test:tst:1805");
        Map<String, ResultEnumeration> definitionResults = new HashMap<>();
        for (int index = 0; index < definitions.getDefinitions().getDefinition().size(); index++)
        {
            ...
            ResultEnumeration resultOfDefinition = ovalInterpreter.evaluateDefinition(definitionId);
            definitionResults.put(definitionId, resultOfDefinition);
        }
        System.out.println("----------------------Definition results------------------------------");
        System.out.println(definitionResults.toString());
        System.out.println("----------------------------------------------------");
        Assert.assertTrue(definitionResults.containsValue(ResultEnumeration.TRUE));
        Assert.assertTrue("Some tests faile because find / -regextype 'posix-extended' -regex '^/etc/i.+/R.+' fails", !definitionResults.containsValue(ResultEnumeration.FALSE));
        Assert.assertTrue(!definitionResults.containsValue(ResultEnumeration.ERROR));
        Assert.assertTrue(!definitionResults.containsValue(ResultEnumeration.NOT_APPLICABLE));
        Assert.assertTrue(!definitionResults.containsValue(ResultEnumeration.NOT_EVALUATED));
        Assert.assertTrue(!definitionResults.containsValue(ResultEnumeration.UNKNOWN));

    }
    
    
    @Test
    public void testUnixFileTestWithSet()
    {
        ...
        String absolutePath = new File("").getAbsolutePath();
        String ovalXMLPath = absolutePath + "/src/main/resources/unit-test-xml/oval-def_set_unix.xml";
        File ovalFile = new File(ovalXMLPath);
