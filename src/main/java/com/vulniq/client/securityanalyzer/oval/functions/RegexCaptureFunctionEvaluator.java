...
public class RegexCaptureFunctionEvaluator extends FunctionEvaluatorBase
{
    private final RegexCaptureFunctionType functionType;
    
    public RegexCaptureFunctionEvaluator(RegexCaptureFunctionType functionType, OVALInterpreter interpreter)
    {
        super(interpreter);
        this.functionType = functionType;
    }
    
    private List<ComponentGroup> evaluateRegexCaptureFunction(List<List<ComponentGroup>> cartesianProduct)
    {
        List<ComponentGroup> rv = new ArrayList<>();
        String regex = functionType.getPattern();
        ...
        for (List<ComponentGroup> componentGroups : cartesianProduct)
        {
            ...
            {
                Matcher matcher = pattern.matcher(group.getValue());
                String match;
                if (matcher.find())
                {
    ...
    protected List<ComponentGroup> evaluate() throws UnexpectedOVALEvaluationError
    {
        ...
        {
            return evaluateRegexCaptureFunction(cartesianProduct);
        }
        else
        {
            ...
            return evaluateRegexCaptureFunction(cartesianWithChildValues);
        }
    }
