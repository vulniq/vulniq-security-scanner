...
public class EscapeRegexFunctionEvaluator extends FunctionEvaluatorBase
{
    private final EscapeRegexFunctionType functionType;
    
    public EscapeRegexFunctionEvaluator(EscapeRegexFunctionType functionType, OVALInterpreter interpreter)
    {
        super(interpreter);
        this.functionType = functionType;
    }
    
    private List<ComponentGroup> evaluateEscapeRegexFunction(List<List<ComponentGroup>> cartesianProduct)
    {
        ...
        for (List<ComponentGroup> groups : cartesianProduct)
        {
            for (ComponentGroup group : groups)
            {
                ...
                String stringToEscape = group.getValue();
                for (int index = 0; index < specialCharacters.length; index++)
                {
                    if (stringToEscape.contains(specialCharacters[index]))
                    {
                        stringToEscape = stringToEscape.replace(specialCharacters[index],
                                                                "\\" + specialCharacters[index]);
                    }
                }
                ComponentGroup returnGroup = new ComponentGroup();
                returnGroup.setDataType(SimpleDatatypeEnumeration.STRING);
                returnGroup.setValue(stringToEscape);
                rv.add(returnGroup);
            }
        }
    ...
    protected List<ComponentGroup> evaluate() throws UnexpectedOVALEvaluationError
    {
        ...
        {
            return evaluateEscapeRegexFunction(cartesianProduct);
        }
        else
        {
            ...
            return evaluateEscapeRegexFunction(cartesianWithChildValues);
        }
    }
